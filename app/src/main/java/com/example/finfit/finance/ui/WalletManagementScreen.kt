    ...
    Box(
        ...
        Column {
            Row(
                ...
                Text("Tổng tài sản", color = Color.White.copy(alpha = 0.8f), fontSize = 14.sp)
                IconButton(onClick = { isHidden = !isHidden }, modifier = Modifier.size(24.dp)) {
                    Icon(
                            imageVector =
            ...
            Text(
                    if (isHidden) "********" else formatCurrency(wallet.totalBalance),
                    color = Color.White,
                    fontSize = 32.sp,
...
fun EditAccountScreen(
    ...
    val account =
            wallet.accounts.find { it.id == accountId }
                    ?: run {
                        onBack()
                        return
                    }
    ...
    var isHidden by remember { mutableStateOf(account.isHidden) }
    ...
    if (showDeleteDialog) {
        AlertDialog(
                ...
                text = {
                    Text(
                            "Bạn có chắc muốn xóa \"${account.displayName}\"?",
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column {
                    ...
                    Text(
                            "Ẩn số dư tài khoản này ở màn hình chính",
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                            fontSize = 11.sp
                    )
                }
...
fun AddAccountScreen(
    ...
    var selectedBank by remember {
        mutableStateOf(SUPPORTED_BANKS.run { find { it.code == "OTHER" } ?: last() })
    }
    var selectedColor by remember {
        mutableStateOf(cardGradientIndex(selectedBank.primaryColorHex))
    }
    var isHidden by remember { mutableStateOf(false) }
    var showBankPicker by remember { mutableStateOf(false) }
                    ...
                    Box(
                            modifier =
                                    Modifier.size(42.dp)
                                            .clip(CircleShape)
                                            .background(
                                                    Color(bank.primaryColorHex).copy(alpha = 0.15f)
                                            ),
    ...
    Row(
        ...
        Box(
                modifier =
                        Modifier.size(44.dp)
                                .clip(CircleShape)
                                .background(Color(bank.primaryColorHex).copy(alpha = 0.2f)),
                contentAlignment = Alignment.Center
    ...
    Box(
        ...
        Row(
            ...
            IconButton(onClick = { onToggle() }, modifier = Modifier.size(32.dp)) {
                Icon(
                        imageVector =
                                if (account.isHidden) Icons.Default.VisibilityOff
                                else Icons.Default.Visibility,
