        ...
        Scaffold(
            ...
            if (showContributionDialog != null) {
                ContributionDialog(
                    ...
                    onAdd = { amount ->
                        val goal = showContributionDialog!!
                        val oldProgress = (goal.currentAmount / goal.targetAmount)
            ...
            // Process Auto-savings on load
            LaunchedEffect(goals) {
                val now = Timestamp.now()
                val ONE_WEEK_MS = 7 * 24 * 60 * 60 * 1000L

                goals.forEach { goal ->
                    if (goal.autoSavingAmount > 0) {
                        val lastAt = goal.lastAutoSavingAt ?: goal.createdAt
                        val diff = now.toDate().time - lastAt.toDate().time

                        if (diff >= ONE_WEEK_MS) {
                            val weeksPass = (diff / ONE_WEEK_MS).toInt()
                            if (weeksPass > 0) {
                                val amountToAdd = weeksPass * goal.autoSavingAmount
                                val updated = goal.copy(
                                    currentAmount = goal.currentAmount + amountToAdd,
                                    lastAutoSavingAt = Timestamp(Date(lastAt.toDate().time + weeksPass * ONE_WEEK_MS))
                                )
                                onSaveGoal(updated)
                            }
                        }
            ...
            Row(
                ...
                IconButton(
                    ...
                    colors = IconButtonDefaults.iconButtonColors(
                        containerColor = if (isCompleted) Color.Gray.copy(alpha = 0.1f) else MaterialTheme.colorScheme.primary.copy(
                            alpha = 0.1f
                        )
                    ),
                    enabled = !isCompleted
...
fun EmptyGoalsState(modifier: Modifier = Modifier) {
    Column(
        ...
        // Thay Lottie bị lỗi bằng Icon ổn định
        Icon(
...
fun AddEditGoalDialog(
    ...
    var autoSaving by remember { mutableStateOf(goal?.autoSavingAmount?.toString() ?: "") }
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = {
                    ...
                    val autoValue = autoSaving.toDoubleOrNull() ?: 0.0
                    onSave(
                        goal?.copy(
                            goalName = name,
                            targetAmount = targetValue,
                            autoSavingAmount = autoValue,
                            iconEmoji = icon,
                            ...
                            iconEmoji = icon,
                            colorHex = selectedColor,
                            lastAutoSavingAt = Timestamp.now()
                        )
                    )
                },
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                OutlinedTextField(
                    value = autoSaving,
                    onValueChange = { autoSaving = it },
                    label = { Text("Tự động nạp mỗi tuần (Không bắt buộc)") },
                    keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
                    modifier = Modifier.fillMaxWidth(),
                    suffix = { Text("đ / tuần") },
                    placeholder = { Text("Ví dụ: 200000") }
                )
